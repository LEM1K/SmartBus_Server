...
public class TrafficAPIReceiver2 extends Thread{
    ...
    public void transportation(String cityCode, String startNodeid, String endNodeid, JSONArray startNodeArrivalBusList, JSONArray endNodeArrivalBusList) {
        ...
        for(int i = 0; i < tmpStartNodeArrivalBusList.size(); i++) {
            System.out.println(tmpStartNodeArrivalBusList.get(i));
        }

        for(int i = 0; i < tmpEndNodeArrivalBusList.size(); i++) {
            System.out.println(tmpEndNodeArrivalBusList.get(i));
        }
        ...
        for(int i = 0; i < tmpEndNodeArrivalBusList.size(); i++) {
            ...
            String currentRouteId = obj.get("routeid").toString();
            getRouteAcctoThrghSttnList(cityCode, currentRouteId);
            putTmp2.put("through", DataCenter.Singleton().accessStationList);
        }

        DataCenter.Singleton().endBusThroughList.add(putTmp2);


        ArrayList<Integer> startNodeOrd = new ArrayList<>();
        ArrayList<Integer> endNodeOrd = new ArrayList<>();

        for(int i = 0; i < DataCenter.Singleton().startBusThroughList.size(); i++) {
            JSONObject obj = (JSONObject) DataCenter.Singleton().startBusThroughList.get(i);

            JSONArray pathArr = (JSONArray) obj.get("through");

            for(int j = 0; j < pathArr.size(); j++) {
                JSONObject stationObj = (JSONObject) pathArr.get(j);
                String stationId = stationObj.get("nodeid").toString();

                if(stationId.equals(startNodeid)) {
                    startNodeOrd.add((Integer) stationObj.get("nodeord"));
                }
            }
        }
        ...
        for(int i = 0; i < DataCenter.Singleton().endBusThroughList.size(); i++) {
            JSONObject obj = (JSONObject) DataCenter.Singleton().endBusThroughList.get(i);
            ...
            for(int j = 0; j < pathArr.size(); j++) {
                ...
                if(stationId.equals(endNodeid)) {
                    endNodeOrd.add((Integer) stationObj.get("nodeord"));
                }
            }
        }

        for(int i = 0; i < startNodeOrd.size(); i++) {
            System.out.println(startNodeOrd.get(i));
        }

        for(int i = 0; i < endNodeOrd.size(); i++) {
            System.out.println(endNodeOrd.get(i));
        }
